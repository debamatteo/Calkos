...
namespace Calkos.web.Areas.Admin.Controllers
{
    ...
    public class ProspettiSystemCoreController : Controller
    {
        /*
         * Questo controller gestisce TUTTO ciò che riguarda SystemCore:
         * - Importazione file Excel SystemCore
         * - Conversione righe Excel → ImportSystemCore
         * - Salvataggio in FileImportato + ProspettoSystemCore
         * - Visualizzazione lista ordini SystemCore
         * 
         * È la versione SPECIALIZZATA del vecchio ProspettiController,
         * ma dedicata SOLO al Mandatario SystemCore.
         */


        /*
         * Il repository SystemCore.
         * 
         * Il controller NON deve conoscere come il repository funziona internamente.
         * Usa SOLO l’interfaccia IProspettoSystemCoreRepository.
         * 
         * In Program.cs abbiamo:
         * services.AddScoped<IProspettoSystemCoreRepository, ProspettoSystemCoreRepository>();
         * 
         * Questo significa:
         * “Quando qualcuno chiede IProspettoSystemCoreRepository, dagli ProspettoSystemCoreRepository”.
         */
        // Il servizio che contiene tutta la pipeline di importazione.
        // Il controller NON fa logica: delega tutto al service.
        private readonly ImportazioneSystemCoreService _ImportazioneSystemCoreService;

        private readonly IProspettoSystemCoreRepository _prospettoRepository;
        private readonly IAgenteRepository _agenteRepository; // serve per lookup agenti
        private readonly IFileBackupService _fileBackupService; // serve per backup file SystemCore
        ...
        public ProspettiSystemCoreController(
            ImportazioneSystemCoreService ImportazioneSystemCoreService,
            IProspettoSystemCoreRepository prospettoRepository,
        ...
        {
            _ImportazioneSystemCoreService = ImportazioneSystemCoreService;
        ...
        // ============================================================
        // INIZIO IMPORTAZIONE SystemCore
        //  GET: PAGINA DI IMPORTAZIONE SystemCore
        // ============================================================
        ...
        public IActionResult Importa(int idMandatario)
        {
            ...
            // Lo salvo in sessione per tutto il flusso SystemCore
        ...
        // ============================================================
        // POST: ESECUZIONE IMPORTAZIONE SystemCore
        // ============================================================
        /*
         * Questo metodo:
         * 1. Legge il file Excel
         * 2. Converte ogni riga in ImportSystemCore
         * 3. Crea un FileImportato
         * 4. Avvia la pipeline di importazione tramite ImportazioneSystemCoreService
        ...
        public IActionResult Importa(IFormFile fileExcel)//, int idMandatario ora uso la sessione
        {
            ...
            var tempFolder = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot", "tempSystemCore");
            Directory.CreateDirectory(tempFolder);

            var tempFileName = $"SystemCore_{DateTime.Now:yyyyMMdd_HHmmss}{Path.GetExtension(fileExcel.FileName)}";
            ...
            // 1. BACKUP FISICO DEL FILE SystemCore (con gestione errori elegante)
            try
            {
                _fileBackupService.BackupSystemcoreAsync(tempFilePath).Wait();
            }
            catch (Exception ex)
            {
                // Non blocchiamo l'importazione SystemCore
                // Registriamo l'errore in TempData (o log)
                TempData["BackupWarning"] = "Backup non eseguito: " + ex.Message;

                // Se hai un logger, puoi usare:
                // _logger.LogError(ex, "Errore durante il backup del file SystemCore.");
            }



            // 2. Leggo l’Excel → List<RigaExcelSystemCore>
            int firstRow = _config.GetValue<int>("ImportazioneSystemCore:FirstRow");
            ...
            //var righe = ExcelHelper.LeggiExcelSystemCore(fileExcel, firstRow, valoriAmmessi);
            //15/04/2026
            List<RigaExcelSystemCore> righe;
            try
            {
                righe = ExcelHelper.LeggiExcelSystemCore(fileExcel, firstRow, valoriAmmessi);
            }
            catch (Exception ex)
            {
            ...
            // 3. Converto ogni riga in ImportSystemCore
            ...
            var righeImportSystemCore = righe.Select(r => Converti(r)).ToList();
            //15/04/2026
            if (righeImportSystemCore.Count == 0)
            {
            ...
            // 5. Avvio importazione ImportazioneSystemCoreService
            ...
            try
            {
                idFile = _ImportazioneSystemCoreService.ImportaFile(file, righeImportSystemCore, utente, IdTipoPagamento);
            }
            catch (Exception ex)
            {
            ...
            //return RedirectToAction("Importa", "ProspettiSystemCore", new { area = "Admin" });
            //Così la pagina Importa si ricarica con il Mandatario corretto
            return RedirectToAction("Importa", "ProspettiSystemCore", new { area = "Admin", idMandatario = file.IdMandatario });

        }


        // ============================================================
        // CONVERSIONE RIGA EXCEL SystemCore → ImportSystemCore
        // ============================================================
        /*
         * Questo metodo converte una riga Excel SystemCore
         * nel modello ImportSystemCore usato dalla pipeline.
         * 
         * È specifico per SystemCore, quindi sta nel controller SystemCore.
         */


        private ImportSystemCore Converti(RigaExcelSystemCore r)
        {
            var i = new ImportSystemCore();
            ...
            i.PercentualeQuotaFissa = r.PercentualeQuotaFissa;
            i.ValoreQuotaFissa = r.ValoreQuotaFissa;
            ...
            i.Perc_Delta_Provvigione = r.Perc_Delta_Provvigione;//26/04/2026
            i.QuotaCentesimiPerKg = r.QuotaCentesimiPerKg;//26/04/2026
            // ============================
            // Date
            // ============================
        ...
        // ============================================================
        // FINE  IMPORTAZIONE SystemCore
        // ============================================================



        //// ============================================================
        //// LISTA ORDINI SystemCore
        //// ============================================================
        ///*
        // * Questa action mostra:
        // * - TUTTI gli ordini SystemCore
             ...
             bool mostraEliminati = false)//25/03/2026 aggiunto filtro fatturata
        {
            ...
            var config = _configService.Load("systemcore");
            ...
            var vm = new ProspettoSystemCoreListaViewModel
            {
        ...
        // ============================================================
        // GET:  DETTAGLIO ORDINE SystemCore (SOLO ADMIN)
        // ============================================================
        ...
        public IActionResult DettaglioOrdine(int? id)
        {
            ...
            {
                var duplicato = JsonConvert.DeserializeObject<ProspettoSystemCore>(TempData["Duplicato"].ToString());
        ...
        // ============================================================
        // POST: SALVATAGGIO DETTAGLIO ORDINE SystemCore (SOLO ADMIN)
        // ============================================================

        [HttpPost]
        [Authorize(Roles = "Admin")]
        public IActionResult DettaglioOrdine(ProspettoSystemCore model)
        {
            /*
            ...
            try
            {
                ...
                else
                {
                    ...
                    model.IdProspettoSystemCore = nuovoId;//03/04/2026 NUOVA
                    ...
                    //model.IdProspettoSystemCore = nuovoId;//03/04/2026 REMMATA
            ...
            //return RedirectToAction("DettaglioOrdine", new { id = model.IdProspettoSystemCore });//03/04/2026 resto nel dettaglio
            //return RedirectToAction("ListaOrdini", new { id = model.IdProspettoSystemCore });//03/04/2026 remmata
        }
        ...
        private void CaricaLookup(ProspettoSystemCore ordine)
        {
            // Agenti
            var agenti = _agenteRepository.GetAll();
        ...
        //// ============================================================
        //// NUOVA RIGA ORDINE SystemCore (SOLO ADMIN)
        //// ============================================================
        public IActionResult Nuovo()
        {
            int idMandatario = HttpContext.Session.GetInt32("IdMandatario") ?? 0;

            var model = new ProspettoSystemCore();

            // --- CHIAVI E METADATI OBBLIGATORI ---
            model.IdMandatario = idMandatario;
            model.IdFileImportato = 0;
            model.IdImportSystemCore = 0;
            model.IdCliente = 0;
            model.IdMateriale = 0;
            ...
            // --- QUANTITÀ E VALORI (DECIMAL 18,6) ---
            model.Quantita = 0m;
           
            model.PrezzoVendita = 0m;
            model.CostoLavorazione = 0m;

            // --- LOGICA PROVVIGIONI SYSTEMCORE ---
            model.PercentualeQuotaFissa = 0m;
            model.ValoreQuotaFissa = 0m;
            ...
            model.Perc_Delta_Provvigione = 0m; // Aggiunto 26/04/2026
            model.QuotaCentesimiPerKg = 0m;    // Aggiunto 26/04/2026
        ...
        public IActionResult DuplicaAjax(int id)//03/04/2026 modificata
        {
            ...
            return Json(new { success = true, newId = nuovo.IdProspettoSystemCore });
        }
            ...
            idFileImportato, int? anno, int? mese, int? fatturata, int? idTipoPagamento)       // 22/05/2026 <-- NUOVO PARAMETRO)
        {
            ...
            var nomeMandatario = "SystemCore"; // oppure recuperalo dal DB
        ...
        public IActionResult ExportPdfProspettoListaOrdini(int? idMandatario, int? idFileImportato, int? anno, int? mese, int? fatturata ,int? idTipoPagamento)       // <-- NUOVO PARAMETRO 22/05/2026
        {
            ...
            var nomeMandatario = "SystemCore"; // oppure lookup DB
            ...
            return File(bytes, "application/pdf", "ProspettoSystemCore.pdf");
        }
        ...
        public IActionResult ListaOrdiniJson(int? idMandatario, int? idFileImportato, int? anno, int? mese, int? fatturata, int? idTipoPagamento,bool mostraEliminati = false)// <-- NUOVO PARAMETRO  22/05/026     
        {
            ...
            // Il metodo GetFiltrati incapsula la chiamata alla Stored Procedure [dbo].[spProspettoSystemCore_GetAllFiltri].
            ...
            // VANTAGGIO ARCHITETTURALE: Restituendo l'intera collezione di oggetti 'ProspettoSystemCore',
            // non vi è accoppiamento forte (Hard Coding) tra le colonne del DB e il backend.
            // Qualsiasi colonna aggiuntiva (es. campi tecnici SystemCore) sarà automaticamente disponibile 

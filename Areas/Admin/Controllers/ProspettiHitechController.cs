...
namespace Calkos.web.Areas.Admin.Controllers
{
    ...
    public class ProspettiHitechController : Controller
    {
        /*
         * Questo controller gestisce TUTTO ciò che riguarda Hitech:
         * - Importazione file Excel Hitech
         * - Conversione righe Excel → ImportHitech
         * - Salvataggio in FileImportato + ProspettoHitech
         * - Visualizzazione lista ordini Hitech
         * 
         * È la versione SPECIALIZZATA del vecchio ProspettiController,
         * ma dedicata SOLO al Mandatario Hitech.
         */


        /*
         * Il repository Hitech.
         * 
         * Il controller NON deve conoscere come il repository funziona internamente.
         * Usa SOLO l’interfaccia IProspettoHitechRepository.
         * 
         * In Program.cs abbiamo:
         * services.AddScoped<IProspettoHitechRepository, ProspettoHitechRepository>();
         * 
         * Questo significa:
         * “Quando qualcuno chiede IProspettoHitechRepository, dagli ProspettoHitechRepository”.
         */
        // Il servizio che contiene tutta la pipeline di importazione.
        // Il controller NON fa logica: delega tutto al service.
        private readonly ImportazioneHitechService _ImportazioneHitechService;

        private readonly IProspettoHitechRepository _prospettoRepository;
        private readonly IAgenteRepository _agenteRepository; // serve per lookup agenti
        private readonly IFileBackupService _fileBackupService; // serve per backup file Hitech
        ...
        public ProspettiHitechController(
            ImportazioneHitechService ImportazioneHitechService,
            IProspettoHitechRepository prospettoRepository,
        ...
        {
            _ImportazioneHitechService = ImportazioneHitechService;
        ...
        // ============================================================
        // INIZIO IMPORTAZIONE Hitech
        //  GET: PAGINA DI IMPORTAZIONE Hitech
        // ============================================================
        ...
        public IActionResult Importa(int idMandatario)
        {
            ...
            // Lo salvo in sessione per tutto il flusso Hitech
        ...
        // ============================================================
        // POST: ESECUZIONE IMPORTAZIONE Hitech
        // ============================================================
        /*
         * Questo metodo:
         * 1. Legge il file Excel
         * 2. Converte ogni riga in ImportHitech
         * 3. Crea un FileImportato
         * 4. Avvia la pipeline di importazione tramite ImportazioneHitechService
        ...
        public IActionResult Importa(IFormFile fileExcel)//, int idMandatario ora uso la sessione
        {
            ...
            var tempFolder = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot", "tempHitech");
            Directory.CreateDirectory(tempFolder);

            var tempFileName = $"Hitech_{DateTime.Now:yyyyMMdd_HHmmss}{Path.GetExtension(fileExcel.FileName)}";
            ...
            // 1. BACKUP FISICO DEL FILE Hitech (con gestione errori elegante)
            try
            {
                _fileBackupService.BackupHitechAsync(tempFilePath).Wait();
            }
            catch (Exception ex)
            {
                // Non blocchiamo l'importazione Hitech
                // Registriamo l'errore in TempData (o log)
                TempData["BackupWarning"] = "Backup non eseguito: " + ex.Message;

                // Se hai un logger, puoi usare:
                // _logger.LogError(ex, "Errore durante il backup del file Hitech.");
            }



            // 2. Leggo l’Excel → List<RigaExcelHitech>
            int firstRow = _config.GetValue<int>("ImportazioneHitech:FirstRow");
            ...
            //var righe = ExcelHelper.LeggiExcelHitech(fileExcel, firstRow, valoriAmmessi);
            //15/04/2026
            List<RigaExcelHitech> righe;
            try
            {
                righe = ExcelHelper.LeggiExcelHitech(fileExcel, firstRow, valoriAmmessi);
            }
            catch (Exception ex)
            {
            ...
            // 3. Converto ogni riga in ImportHitech
            ...
            var righeImportHitech = righe.Select(r => Converti(r)).ToList();
            //15/04/2026
            if (righeImportHitech.Count == 0)
            {
            ...
            // 5. Avvio importazione ImportazioneHitechService
            ...
            //15/04/2026
            //int idFile = _ImportazioneHitechService.ImportaFile(file, righeImportHitech, utente, IdTipoPagamento); 
            int idFile;
            try
            {
                idFile = _ImportazioneHitechService.ImportaFile(file, righeImportHitech, utente, IdTipoPagamento);
            }
            catch (Exception ex)
            {
            ...
            //return RedirectToAction("Importa", "ProspettiHitech", new { area = "Admin" });
            //Così la pagina Importa si ricarica con il Mandatario corretto
            return RedirectToAction("Importa", "ProspettiHitech", new { area = "Admin", idMandatario = file.IdMandatario });

        }


        // ============================================================
        // CONVERSIONE RIGA EXCEL Hitech → ImportHitech
        // ============================================================
        /*
         * Questo metodo converte una riga Excel Hitech
         * nel modello ImportHitech usato dalla pipeline.
         * 
         * È specifico per Hitech, quindi sta nel controller Hitech.
         */


        private ImportHitech Converti(RigaExcelHitech r)
        {
            var i = new ImportHitech();
            ...
            i.PrezzoPraticato = r.PrezzoPraticato;
            ...
            // Date
            // ============================
            i.DataDoc = r.DataDoc;//21/04/2026
            i.DataConsegna = r.DataConsegna;

            // ============================
            // Agente (stringa Excel)
            // ============================
            i.Agente = r.Agente;
        ...
        public IActionResult ListaOrdini(int? idMandatario, int? idFileImportato, int? anno, int? mese, int? fatturata, bool mostraEliminati = false)//25/03/2026 aggiunto filtro fatturata
        {
            ...
            var config = _configService.Load("hitech");
            ...
            var vm = new ProspettoHitechListaViewModel
            {
        ...
        // ============================================================
        // GET:  DETTAGLIO ORDINE Hitech (SOLO ADMIN)
        // ============================================================
        ...
        public IActionResult DettaglioOrdine(int? id)
        {
            ...
            {
                var duplicato = JsonConvert.DeserializeObject<ProspettoHitech>(TempData["Duplicato"].ToString());
        ...
        // ============================================================
        // POST: SALVATAGGIO DETTAGLIO ORDINE Hitech (SOLO ADMIN)
        // ============================================================

        [HttpPost]
        [Authorize(Roles = "Admin")]
        public IActionResult DettaglioOrdine(ProspettoHitech model)
        {
            /*
            ...
            try
            {
                ...
                else
                {
                    ...
                    model.IdProspettoHitech = nuovoId;//03/04/2026 NUOVA
                    ...
                    //model.IdProspettoHitech = nuovoId;//03/04/2026 REMMATA
            ...
            //return RedirectToAction("DettaglioOrdine", new { id = model.IdProspettoHitech });//03/04/2026 resto nel dettaglio
            //return RedirectToAction("ListaOrdini", new { id = model.IdProspettoHitech });//03/04/2026 remmata
        }
        ...
        private void CaricaLookup(ProspettoHitech ordine)
        {
            // Agenti
            var agenti = _agenteRepository.GetAll();
        ...
        // ============================================================
        // NUOVA RIGA ORDINE Hitech (SOLO ADMIN)
        // ============================================================
        ...
        public IActionResult Nuovo()
        {
            ...
            // --- DATE E PERIODI ---
            model.DataDoc = DateTime.Today; // Data 20/04/2026
        ...
        public IActionResult DuplicaAjax(int id)//03/04/2026 modificata
        {
            ...
            return Json(new { success = true, newId = nuovo.IdProspettoHitech });
        }
        ...
        public IActionResult ExportExcelProspettoListaOrdini(int? idMandatario, int? idFileImportato, int? anno, int? mese, int? fatturata)
        {
            ...
            var nomeMandatario = "Hitech"; // oppure recuperalo dal DB
        ...
        public IActionResult ExportPdfProspettoListaOrdini(int? idMandatario, int? idFileImportato, int? anno, int? mese, int? fatturata)
        {
            ...
            var nomeMandatario = "Hitech"; // oppure lookup DB
            ...
            return File(bytes, "application/pdf", "ProspettoHitech.pdf");
        }
        ...
        public IActionResult ListaOrdiniJson(int? idMandatario, int? idFileImportato, int? anno, int? mese, int? fatturata, bool mostraEliminati = false)
        {
            ...
            // Il metodo GetFiltrati incapsula la chiamata alla Stored Procedure [dbo].[spProspettoHitech_GetAllFiltri].
            ...
            // VANTAGGIO ARCHITETTURALE: Restituendo l'intera collezione di oggetti 'ProspettoHitech',
            // non vi è accoppiamento forte (Hard Coding) tra le colonne del DB e il backend.
            // Qualsiasi colonna aggiuntiva (es. campi tecnici Hitech) sarà automaticamente disponibile 

...
namespace Calkos.web.Areas.Admin.Controllers
{
    ...
    public class ProspettiGuerzoniController : Controller
    {
        /*
         * Questo controller gestisce TUTTO ciò che riguarda Guerzoni:
         * - Importazione file Excel Guerzoni
         * - Conversione righe Excel → ImportGuerzoni
         * - Salvataggio in FileImportato + ProspettoGuerzoni
         * - Visualizzazione lista ordini Guerzoni
         * 
         * È la versione SPECIALIZZATA del vecchio ProspettiController,
         * ma dedicata SOLO al Mandatario Guerzoni.
         */


        /*
         * Il repository Guerzoni.
         * 
         * Il controller NON deve conoscere come il repository funziona internamente.
         * Usa SOLO l’interfaccia IProspettoGuerzoniRepository.
         * 
         * In Program.cs abbiamo:
         * services.AddScoped<IProspettoGuerzoniRepository, ProspettoGuerzoniRepository>();
         * 
         * Questo significa:
         * “Quando qualcuno chiede IProspettoGuerzoniRepository, dagli ProspettoGuerzoniRepository”.
         */
        // Il servizio che contiene tutta la pipeline di importazione.
        // Il controller NON fa logica: delega tutto al service.
        private readonly ImportazioneGuerzoniService _ImportazioneGuerzoniService;

        private readonly IProspettoGuerzoniRepository _prospettoRepository;
        private readonly IAgenteRepository _agenteRepository; // serve per lookup agenti
        private readonly IFileBackupService _fileBackupService; // serve per backup file Guerzoni
        ...
        public ProspettiGuerzoniController(
            ImportazioneGuerzoniService ImportazioneGuerzoniService,
            IProspettoGuerzoniRepository prospettoRepository,
        ...
        {
            _ImportazioneGuerzoniService = ImportazioneGuerzoniService;
        ...
        // ============================================================
        // INIZIO IMPORTAZIONE Guerzoni
        //  GET: PAGINA DI IMPORTAZIONE Guerzoni
        // ============================================================
        ...
        public IActionResult Importa(int idMandatario)
        {
            ...
            // Lo salvo in sessione per tutto il flusso Guerzoni
        ...
        // ============================================================
        // POST: ESECUZIONE IMPORTAZIONE Guerzoni
        // ============================================================
        /*
         * Questo metodo:
         * 1. Legge il file Excel
         * 2. Converte ogni riga in ImportGuerzoni
         * 3. Crea un FileImportato
         * 4. Avvia la pipeline di importazione tramite ImportazioneGuerzoniService
        ...
        public IActionResult Importa(IFormFile fileExcel)//, int idMandatario ora uso la sessione
        {
            ...
            var tempFolder = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot", "tempGuerzoni");
            Directory.CreateDirectory(tempFolder);

            var tempFileName = $"Guerzoni_{DateTime.Now:yyyyMMdd_HHmmss}{Path.GetExtension(fileExcel.FileName)}";
            ...
            // 1. BACKUP FISICO DEL FILE Guerzoni (con gestione errori elegante)
            try
            {
                _fileBackupService.BackupGuerzoniAsync(tempFilePath).Wait();
            }
            catch (Exception ex)
            {
                // Non blocchiamo l'importazione Guerzoni
                // Registriamo l'errore in TempData (o log)
                TempData["BackupWarning"] = "Backup non eseguito: " + ex.Message;

                // Se hai un logger, puoi usare:
                // _logger.LogError(ex, "Errore durante il backup del file Guerzoni.");
            }



            // 2. Leggo l’Excel → List<RigaExcelGuerzoni>
            int firstRow = _config.GetValue<int>("ImportazioneGuerzoni:FirstRow");
            ...
            //var righe = ExcelHelper.LeggiExcelGuerzoni(fileExcel, firstRow, valoriAmmessi);
            //15/04/2026
            List<RigaExcelGuerzoni> righe;
            try
            {
                righe = ExcelHelper.LeggiExcelGuerzoni(fileExcel, firstRow, valoriAmmessi);
            }
            catch (Exception ex)
            {
            ...
            // 3. Converto ogni riga in ImportGuerzoni
            ...
            var righeImportGuerzoni = righe.Select(r => Converti(r)).ToList();
            //15/04/2026
            if (righeImportGuerzoni.Count == 0)
            {
            ...
            // 5. Avvio importazione ImportazioneGuerzoniService
            ...
            //15/04/2026
            //int idFile = _ImportazioneGuerzoniService.ImportaFile(file, righeImportGuerzoni, utente, IdTipoPagamento); 
            int idFile;
            try
            {
                idFile = _ImportazioneGuerzoniService.ImportaFile(file, righeImportGuerzoni, utente, IdTipoPagamento);
            }
            catch (Exception ex)
            {
            ...
            //return RedirectToAction("Importa", "ProspettiGuerzoni", new { area = "Admin" });
            //Così la pagina Importa si ricarica con il Mandatario corretto
            return RedirectToAction("Importa", "ProspettiGuerzoni", new { area = "Admin", idMandatario = file.IdMandatario });

        }


        // ============================================================
        // CONVERSIONE RIGA EXCEL Guerzoni → ImportGuerzoni
        // ============================================================
        /*
         * Questo metodo converte una riga Excel Guerzoni
         * nel modello ImportGuerzoni usato dalla pipeline.
         * 
         * È specifico per Guerzoni, quindi sta nel controller Guerzoni.
         */


        private ImportGuerzoni Converti(RigaExcelGuerzoni r)
        {
            var i = new ImportGuerzoni();
            // ============================
            // Campi base
            // ============================
            ...
            // Quantità e prezzi
            // ============================
            i.Quantita = r.Quantita;
            i.PrezzoUnitario = r.PrezzoUnitario;
            i.Imponibile = r.Imponibile;
            i.Perc_Delta_Provvigione = r.Perc_Delta_Provvigione;//29/04/2026;
        ...
        // ============================================================
        // FINE  IMPORTAZIONE Guerzoni
        // ============================================================



        //// ============================================================
        //// LISTA ORDINI Guerzoni
        //// ============================================================
        ///*
        // * Questa action mostra:
        // * - TUTTI gli ordini Guerzoni
        ...
        public IActionResult ListaOrdini(int? idMandatario, int? idFileImportato, int? anno, int? mese, int? fatturata, bool mostraEliminati = false)//25/03/2026 aggiunto filtro fatturata
        {
            ...
            var config = _configService.Load("guerzoni");
            ...
            var vm = new ProspettoGuerzoniListaViewModel
            {
        ...
        // ============================================================
        // GET:  DETTAGLIO ORDINE Guerzoni (SOLO ADMIN)
        // ============================================================
        ...
        public IActionResult DettaglioOrdine(int? id)
        {
            ...
            {
                var duplicato = JsonConvert.DeserializeObject<ProspettoGuerzoni>(TempData["Duplicato"].ToString());
        ...
        // ============================================================
        // POST: SALVATAGGIO DETTAGLIO ORDINE Guerzoni (SOLO ADMIN)
        // ============================================================

        [HttpPost]
        [Authorize(Roles = "Admin")]
        public IActionResult DettaglioOrdine(ProspettoGuerzoni model)
        {
            /*
            ...
            try
            {
                ...
                else
                {
                    ...
                    model.IdProspettoGuerzoni = nuovoId;//03/04/2026 NUOVA
                    ...
                    //model.IdProspettoGuerzoni = nuovoId;//03/04/2026 REMMATA
            ...
            //return RedirectToAction("DettaglioOrdine", new { id = model.IdProspettoGuerzoni });//03/04/2026 resto nel dettaglio
            //return RedirectToAction("ListaOrdini", new { id = model.IdProspettoGuerzoni });//03/04/2026 remmata
        }
        ...
        private void CaricaLookup(ProspettoGuerzoni ordine)
        {
            // Agenti
            var agenti = _agenteRepository.GetAll();
        ...
        //// ============================================================
        //// NUOVA RIGA ORDINE Guerzoni (SOLO ADMIN)
        //// ============================================================
        ...
        public IActionResult Nuovo()
        {
            ...
            var model = new ProspettoGuerzoni();

            // --- CHIAVI E METADATI OBBLIGATORI ---
            model.IdMandatario = idMandatario;
            model.IdFileImportato = 0;
            model.IdImportGuerzoni = 0;
            model.IdCliente = 0;
            model.IdMateriale = 0;
            ...
            model.Quantita = 0m;
            model.PrezzoUnitario = 0m;
            model.Imponibile = 0m;
            model.Perc_Delta_Provvigione = 0m; // Aggiunto 29/04/2026
        ...
        public IActionResult DuplicaAjax(int id)//03/04/2026 modificata
        {
            ...
            return Json(new { success = true, newId = nuovo.IdProspettoGuerzoni });
        }
        ...
        public IActionResult ExportExcelProspettoListaOrdini(int? idMandatario, int? idFileImportato, int? anno, int? mese, int? fatturata)
        {
            ...
            var nomeMandatario = "Guerzoni"; // oppure recuperalo dal DB
        ...
        public IActionResult ExportPdfProspettoListaOrdini(int? idMandatario, int? idFileImportato, int? anno, int? mese, int? fatturata)
        {
            ...
            var nomeMandatario = "Guerzoni"; // oppure lookup DB
            ...
            return File(bytes, "application/pdf", "ProspettoGuerzoni.pdf");
        }
        ...
        public IActionResult ListaOrdiniJson(int? idMandatario, int? idFileImportato, int? anno, int? mese, int? fatturata, bool mostraEliminati = false)
        {
            ...
            // Il metodo GetFiltrati incapsula la chiamata alla Stored Procedure [dbo].[spProspettoGuerzoni_GetAllFiltri].
            ...
            // VANTAGGIO ARCHITETTURALE: Restituendo l'intera collezione di oggetti 'ProspettoGuerzoni',
            // non vi è accoppiamento forte (Hard Coding) tra le colonne del DB e il backend.
            // Qualsiasi colonna aggiuntiva (es. campi tecnici Guerzoni) sarà automaticamente disponibile 

...
namespace Calkos.web.Areas.Admin.Controllers
{
    ...
    public class ProspettiTradingAndConsultingController : Controller
    {
        /*
         * Questo controller gestisce TUTTO ciò che riguarda TradingAndConsulting:
         * - Importazione file Excel TradingAndConsulting
         * - Conversione righe Excel → ImportTradingAndConsulting
         * - Salvataggio in FileImportato + ProspettoTradingAndConsulting
         * - Visualizzazione lista ordini TradingAndConsulting
         * 
         * È la versione SPECIALIZZATA del vecchio ProspettiController,
         * ma dedicata SOLO al Mandatario TradingAndConsulting.
         */


        /*
         * Il repository TradingAndConsulting.
         * 
         * Il controller NON deve conoscere come il repository funziona internamente.
         * Usa SOLO l’interfaccia IProspettoTradingAndConsultingRepository.
         * 
         * In Program.cs abbiamo:
         * services.AddScoped<IProspettoTradingAndConsultingRepository, ProspettoTradingAndConsultingRepository>();
         * 
         * Questo significa:
         * “Quando qualcuno chiede IProspettoTradingAndConsultingRepository, dagli ProspettoTradingAndConsultingRepository”.
         */
        // Il servizio che contiene tutta la pipeline di importazione.
        // Il controller NON fa logica: delega tutto al service.
        private readonly ImportazioneTradingAndConsultingService _ImportazioneTradingAndConsultingService;

        private readonly IProspettoTradingAndConsultingRepository _prospettoRepository;
        private readonly IAgenteRepository _agenteRepository; // serve per lookup agenti
        private readonly IFileBackupService _fileBackupService; // serve per backup file TradingAndConsulting
        ...
        public ProspettiTradingAndConsultingController(
            ImportazioneTradingAndConsultingService ImportazioneTradingAndConsultingService,
            IProspettoTradingAndConsultingRepository prospettoRepository,
        ...
        {
            _ImportazioneTradingAndConsultingService = ImportazioneTradingAndConsultingService;
        ...
        // ============================================================
        // INIZIO IMPORTAZIONE TradingAndConsulting
        //  GET: PAGINA DI IMPORTAZIONE TradingAndConsulting
        // ============================================================
        ...
        public IActionResult Importa(int idMandatario)
        {
            ...
            // Lo salvo in sessione per tutto il flusso TradingAndConsulting
        ...
        // ============================================================
        // POST: ESECUZIONE IMPORTAZIONE TradingAndConsulting
        // ============================================================
        /*
         * Questo metodo:
         * 1. Legge il file Excel
         * 2. Converte ogni riga in ImportTradingAndConsulting
         * 3. Crea un FileImportato
         * 4. Avvia la pipeline di importazione tramite ImportazioneTradingAndConsultingService
        ...
        public IActionResult Importa(IFormFile fileExcel)//, int idMandatario ora uso la sessione
        {
            ...
            var tempFolder = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot", "tempTradingAndConsulting");
            Directory.CreateDirectory(tempFolder);

            var tempFileName = $"TradingAndConsulting_{DateTime.Now:yyyyMMdd_HHmmss}{Path.GetExtension(fileExcel.FileName)}";
            ...
            // 1. BACKUP FISICO DEL FILE TradingAndConsulting (con gestione errori elegante)
            try
            {
                _fileBackupService.BackupTradingAndConsultingAsync(tempFilePath).Wait();
            }
            catch (Exception ex)
            {
                // Non blocchiamo l'importazione TradingAndConsulting
                // Registriamo l'errore in TempData (o log)
                TempData["BackupWarning"] = "Backup non eseguito: " + ex.Message;

                // Se hai un logger, puoi usare:
                // _logger.LogError(ex, "Errore durante il backup del file TradingAndConsulting.");
            }



            // 2. Leggo l’Excel → List<RigaExcelTradingAndConsulting>
            int firstRow = _config.GetValue<int>("ImportazioneTradingAndConsulting:FirstRow");
            ...
            //var righe = ExcelHelper.LeggiExcelTradingAndConsulting(fileExcel, firstRow, valoriAmmessi);
            //15/04/2026
            List<RigaExcelTradingAndConsulting> righe;
            try
            {
                righe = ExcelHelper.LeggiExcelTradingAndConsulting(fileExcel, firstRow, valoriAmmessi);
            }
            catch (Exception ex)
            {
            ...
            // 3. Converto ogni riga in ImportTradingAndConsulting
            ...
            var righeImportTradingAndConsulting = righe.Select(r => Converti(r)).ToList();
            //15/04/2026
            if (righeImportTradingAndConsulting.Count == 0)
            {
            ...
            // 5. Avvio importazione ImportazioneTradingAndConsultingService
            ...
            //15/04/2026
            //int idFile = _ImportazioneTradingAndConsultingService.ImportaFile(file, righeImportTradingAndConsulting, utente, IdTipoPagamento); 
            int idFile;
            try
            {
                idFile = _ImportazioneTradingAndConsultingService.ImportaFile(file, righeImportTradingAndConsulting, utente, IdTipoPagamento);
            }
            catch (Exception ex)
            {
            ...
            //return RedirectToAction("Importa", "ProspettiTradingAndConsulting", new { area = "Admin" });
            //Così la pagina Importa si ricarica con il Mandatario corretto
            return RedirectToAction("Importa", "ProspettiTradingAndConsulting", new { area = "Admin", idMandatario = file.IdMandatario });

        }


        // ============================================================
        // CONVERSIONE RIGA EXCEL TradingAndConsulting → ImportTradingAndConsulting
        // ============================================================
        /*
         * Questo metodo converte una riga Excel TradingAndConsulting
         * nel modello ImportTradingAndConsulting usato dalla pipeline.
         * 
         * È specifico per TradingAndConsulting, quindi sta nel controller TradingAndConsulting.
         */


        private ImportTradingAndConsulting Converti(RigaExcelTradingAndConsulting r)
        {
            var i = new ImportTradingAndConsulting();
            ...
            // Prezzi
            // ============================
            i.PrezzoBase = r.PrezzoBase;
            i.PrezzoEuroRiferimento = r.PrezzoEuroRiferimento;//21/04/2026
            i.PrezzoVendita = r.PrezzoVendita;
            i.PrezzoBaseConLavorazione = r.PrezzoBaseConLavorazione;

            // ============================
            // Provvigioni fisse
            // ============================
            i.ProvvigioneFissa01_05 = r.ProvvigioneFissa01_05;
            i.ProvvigioneFissa03 = r.ProvvigioneFissa03;
            i.Perc_Delta_Provvigione = r.Perc_Delta_Provvigione;//30/04/2026
            i.TotaleProvvigioneFissa = r.TotaleProvvigioneFissa;
            ...
            i.CIT = r.CIT;
            i.Delta = r.Delta;
            i.TotaleProvvigioneVariabile = r.TotaleProvvigioneVariabile;
            i.TotaleProvvigioneFattura = r.TotaleProvvigioneFattura;

            // ============================
            // Date
            // ============================
            i.DataConsegna = r.DataConsegna;
        ...
        // ============================================================
        // FINE  IMPORTAZIONE TradingAndConsulting
        // ============================================================



        //// ============================================================
        //// LISTA ORDINI TradingAndConsulting
        //// ============================================================
        ///*
        // * Questa action mostra:
        // * - TUTTI gli ordini TradingAndConsulting
        ...
        public IActionResult ListaOrdini(int? idMandatario, int? idFileImportato, int? anno, int? mese, int? fatturata, bool mostraEliminati = false)//25/03/2026 aggiunto filtro fatturata
        {
            ...
            var config = _configService.Load("tradingandconsulting");
            ...
            var vm = new ProspettoTradingAndConsultingListaViewModel
            {
        ...
        // ============================================================
        // GET:  DETTAGLIO ORDINE TradingAndConsulting (SOLO ADMIN)
        // ============================================================
        ...
        public IActionResult DettaglioOrdine(int? id)
        {
            ...
            {
                var duplicato = JsonConvert.DeserializeObject<ProspettoTradingAndConsulting>(TempData["Duplicato"].ToString());
        ...
        // ============================================================
        // POST: SALVATAGGIO DETTAGLIO ORDINE TradingAndConsulting (SOLO ADMIN)
        // ============================================================

        [HttpPost]
        [Authorize(Roles = "Admin")]
        public IActionResult DettaglioOrdine(ProspettoTradingAndConsulting model)
        {
            /*
            ...
            try
            {
                ...
                if (model.IdProspettoTradingAndConsulting != 0)
                {
                ...
                else
                {
                    ...
                    model.IdProspettoTradingAndConsulting = nuovoId;//03/04/2026 NUOVA
                    ...
                    //model.IdProspettoTradingAndConsulting = nuovoId;//03/04/2026 REMMATA
            ...
            //return RedirectToAction("DettaglioOrdine", new { id = model.IdProspettoTradingAndConsulting });//03/04/2026 resto nel dettaglio
            //return RedirectToAction("ListaOrdini", new { id = model.IdProspettoTradingAndConsulting });//03/04/2026 remmata
        }
        ...
        private void CaricaLookup(ProspettoTradingAndConsulting ordine)
        {
            // Agenti
            var agenti = _agenteRepository.GetAll();
        ...
        //// ============================================================
        //// NUOVA RIGA ORDINE TradingAndConsulting (SOLO ADMIN)
        //// ============================================================
        public IActionResult Nuovo()
        {
            int idMandatario = HttpContext.Session.GetInt32("IdMandatario") ?? 0;

            var model = new ProspettoTradingAndConsulting();

            // --- CHIAVI E METADATI OBBLIGATORI ---
            model.IdMandatario = idMandatario;
            model.IdFileImportato = 0;
            model.IdImportTradingAndConsulting = 0;
            model.IdCliente = 0;
            model.IdMateriale = 0;
            ...
            // --- DATE E PERIODI ---
            model.DataConsegna = DateTime.Today;
            ...
            // --- QUANTITÀ E PREZZI (DECIMAL 18,6 PER CALCOLI JS) ---
            model.Quantita = 0m;
            model.CostoLavorazione = 0m;
            model.PrezzoBase = 0m;
            model.PrezzoVendita = 0m;
            model.PrezzoBaseConLavorazione = 0m;
            model.PrezzoEuroRiferimento = 0m; // Aggiunto 20/04/2026

            // --- LOGICA PROVVIGIONI TradingAndConsulting (EVITA NaN/NULL) ---
            model.ProvvigioneFissa01_05 = 0m;
            model.ProvvigioneFissa03 = 0m;
            model.Perc_Delta_Provvigione = 0m;//30/04/2026
            model.TotaleProvvigioneFissa = 0m;
            model.CIT = 0m;
            model.Delta = 0m;
            model.TotaleProvvigioneVariabile = 0m;
            model.TotaleProvvigioneFattura = 0m;
            model.ProvvigioneAgente = 0m;

            // --- STATI E FLAG ---
            model.Fatturare = false;
            model.Fatturata = 0;
        ...
        public IActionResult DuplicaAjax(int id)//03/04/2026 modificata
        {
            ...
            return Json(new { success = true, newId = nuovo.IdProspettoTradingAndConsulting });
        }
        ...
        public IActionResult ExportExcelProspettoListaOrdini(int? idMandatario, int? idFileImportato, int? anno, int? mese, int? fatturata)
        {
            ...
            var nomeMandatario = "TradingAndConsulting"; // oppure recuperalo dal DB
            ...
            return File(bytes,
                ...
                "ProspettoTradingAndConsulting.xlsx");
        }
        ...
        public IActionResult ExportPdfProspettoListaOrdini(int? idMandatario, int? idFileImportato, int? anno, int? mese, int? fatturata)
        {
            ...
            var nomeMandatario = "TradingAndConsulting"; // oppure lookup DB

            var bytes = _pdfExportService.CreaPdfProspettoListaOrdini(
                prospetti,
                config.Columns,
                "Prospetto TradingAndConsulting",
                nomeMandatario,
                mese ?? 0,
                anno ?? 0
            );

            return File(bytes, "application/pdf", "ProspettoTradingAndConsulting.pdf");
        }
        ...
        public IActionResult ListaOrdiniJson(int? idMandatario, int? idFileImportato, int? anno, int? mese, int? fatturata, bool mostraEliminati = false)
        {
            ...
            // VANTAGGIO ARCHITETTURALE: Restituendo l'intera collezione di oggetti 'ProspettoTradingAndConsulting',
            // non vi è accoppiamento forte (Hard Coding) tra le colonne del DB e il backend.
            // Qualsiasi colonna aggiuntiva (es. campi tecnici TradingAndConsulting) sarà automaticamente disponibile 

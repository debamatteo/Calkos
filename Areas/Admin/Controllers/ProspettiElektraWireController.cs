...
namespace Calkos.web.Areas.Admin.Controllers
{
    ...
    public class ProspettiElektraWireController : Controller
    {
        /*
         * Questo controller gestisce TUTTO ciò che riguarda ElektraWire:
         * - Importazione file Excel ElektraWire
         * - Conversione righe Excel → ImportElektraWire
         * - Salvataggio in FileImportato + ProspettoElektraWire
         * - Visualizzazione lista ordini ElektraWire
         * 
         * È la versione SPECIALIZZATA del vecchio ProspettiController,
         * ma dedicata SOLO al Mandatario ElektraWire.
         */


        /*
         * Il repository ElektraWire.
         * 
         * Il controller NON deve conoscere come il repository funziona internamente.
         * Usa SOLO l’interfaccia IProspettoElektraWireRepository.
         * 
         * In Program.cs abbiamo:
         * services.AddScoped<IProspettoElektraWireRepository, ProspettoElektraWireRepository>();
         * 
         * Questo significa:
         * “Quando qualcuno chiede IProspettoElektraWireRepository, dagli ProspettoElektraWireRepository”.
         */
        // Il servizio che contiene tutta la pipeline di importazione.
        // Il controller NON fa logica: delega tutto al service.
        private readonly ImportazioneElektraWireService _ImportazioneElektraWireService;

        private readonly IProspettoElektraWireRepository _prospettoRepository;
        private readonly IAgenteRepository _agenteRepository; // serve per lookup agenti
        private readonly IFileBackupService _fileBackupService; // serve per backup file ElektraWire
        ...
        public ProspettiElektraWireController(
            ImportazioneElektraWireService ImportazioneElektraWireService,
            IProspettoElektraWireRepository prospettoRepository,
        ...
        {
            _ImportazioneElektraWireService = ImportazioneElektraWireService;
        ...
        // ============================================================
        // INIZIO IMPORTAZIONE ElektraWire
        //  GET: PAGINA DI IMPORTAZIONE ElektraWire
        // ============================================================
        ...
        public IActionResult Importa(int idMandatario)
        {
            ...
            // Lo salvo in sessione per tutto il flusso ElektraWire
        ...
        // ============================================================
        // POST: ESECUZIONE IMPORTAZIONE ElektraWire
        // ============================================================
        /*
         * Questo metodo:
         * 1. Legge il file Excel
         * 2. Converte ogni riga in ImportElektraWire
         * 3. Crea un FileImportato
         * 4. Avvia la pipeline di importazione tramite ImportazioneElektraWireService
        ...
        public IActionResult Importa(IFormFile fileExcel)//, int idMandatario ora uso la sessione
        {
            ...
            var tempFolder = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot", "tempElektraWire");
            Directory.CreateDirectory(tempFolder);

            var tempFileName = $"ElektraWire_{DateTime.Now:yyyyMMdd_HHmmss}{Path.GetExtension(fileExcel.FileName)}";
            ...
            // 1. BACKUP FISICO DEL FILE ElektraWire (con gestione errori elegante)
            try
            {
                _fileBackupService.BackupElektraWireAsync(tempFilePath).Wait();
            }
            catch (Exception ex)
            {
                // Non blocchiamo l'importazione ElektraWire
                // Registriamo l'errore in TempData (o log)
                TempData["BackupWarning"] = "Backup non eseguito: " + ex.Message;

                // Se hai un logger, puoi usare:
                // _logger.LogError(ex, "Errore durante il backup del file ElektraWire.");
            }



            // 2. Leggo l’Excel → List<RigaExcelElektraWire>
            int firstRow = _config.GetValue<int>("ImportazioneElektraWire:FirstRow");
            ...
            //var righe = ExcelHelper.LeggiExcelElektraWire(fileExcel, firstRow, valoriAmmessi);
            //15/04/2026
            List<RigaExcelElektraWire> righe;
            try
            {
                righe = ExcelHelper.LeggiExcelElektraWire(fileExcel, firstRow, valoriAmmessi);
            }
            catch (Exception ex)
            {
            ...
            // 3. Converto ogni riga in ImportElektraWire
            ...
            var righeImportElektraWire = righe.Select(r => Converti(r)).ToList();
            //15/04/2026
            if (righeImportElektraWire.Count == 0)
            {
            ...
            // 5. Avvio importazione ImportazioneElektraWireService
            ...
            //15/04/2026
            //int idFile = _ImportazioneElektraWireService.ImportaFile(file, righeImportElektraWire, utente, IdTipoPagamento); 
            int idFile;
            try
            {
                idFile = _ImportazioneElektraWireService.ImportaFile(file, righeImportElektraWire, utente, IdTipoPagamento);
            }
            catch (Exception ex)
            {
            ...
            //return RedirectToAction("Importa", "ProspettiElektraWire", new { area = "Admin" });
            //Così la pagina Importa si ricarica con il Mandatario corretto
            return RedirectToAction("Importa", "ProspettiElektraWire", new { area = "Admin", idMandatario = file.IdMandatario });

        }


        // ============================================================
        // CONVERSIONE RIGA EXCEL ElektraWire → ImportElektraWire
        // ============================================================
        /*
         * Questo metodo converte una riga Excel ElektraWire
         * nel modello ImportElektraWire usato dalla pipeline.
         * 
         * È specifico per ElektraWire, quindi sta nel controller ElektraWire.
         */

        private ImportElektraWire Converti(RigaExcelElektraWire r)
        {
            var i = new ImportElektraWire();
            ...
            i.DeltaMargine = r.DeltaMargine;
            i.Perc_Delta_Provvigione = r.Perc_Delta_Provvigione;//27/04/2026

            // ============================
            // Provvigioni fisse 1,5%
            // ============================
            i.ValUnitProvvigioneFissa1_5 = r.ValUnitProvvigioneFissa1_5;
            i.Delta1ProvvigioneFissa1_5 = r.Delta1ProvvigioneFissa1_5;
            i.Delta2ProvvigioneFissa1_5 = r.Delta2ProvvigioneFissa1_5;
            i.TotProvvigioneFissa1_5 = r.TotProvvigioneFissa1_5;

            // ============================
            // Provvigioni fisse 3%
            // ============================
            i.ValUnitProvvigioneFissa_03 = r.ValUnitProvvigioneFissa_03;
            i.Delta1ProvvigioneFissa_03 = r.Delta1ProvvigioneFissa_03;
            i.Delta2ProvvigioneFissa_03 = r.Delta2ProvvigioneFissa_03;
            i.TotProvvigioneFissa_03 = r.TotProvvigioneFissa_03;

            // ============================
            // Quote fisse
            // ============================
            i.ValQuotaFissa_1_5 = r.ValQuotaFissa_1_5;
            i.ValQuotaFissa_03 = r.ValQuotaFissa_03;
            i.TotaleFissoProvvigione = r.TotaleFissoProvvigione;

            // ============================
            // Variabile
            // ============================
            i.DeltaResiduoVariabile = r.DeltaResiduoVariabile;
            i.TotaleVariabileProvvigione = r.TotaleVariabileProvvigione;
            i.TotaleSommaProvvigioni = r.TotaleSommaProvvigioni;
        ...
        // ============================================================
        // FINE  IMPORTAZIONE ElektraWire
        // ============================================================



        // ============================================================
        // LISTA ORDINI ElektraWire
        // ============================================================
        /*
         * Questa action mostra:
         * - TUTTI gli ordini ElektraWire
        ...
        public IActionResult ListaOrdini(int? idMandatario, int? idFileImportato, int? anno, int? mese, int? fatturata, bool mostraEliminati = false)//25/03/2026 aggiunto filtro fatturata
        {
            ...
            var config = _configService.Load("elektrawire");
            ...
            var vm = new ProspettoElektraWireListaViewModel
            {
        ...
        //// ============================================================
        //// GET:  DETTAGLIO ORDINE ElektraWire (SOLO ADMIN)
        //// ============================================================
        ...
        public IActionResult DettaglioOrdine(int? id)
        {
            ...
            {
                var duplicato = JsonConvert.DeserializeObject<ProspettoElektraWire>(TempData["Duplicato"].ToString());
        ...
        // ============================================================
        // POST: SALVATAGGIO DETTAGLIO ORDINE ElektraWire (SOLO ADMIN)
        // ============================================================

        [HttpPost]
        [Authorize(Roles = "Admin")]
        public IActionResult DettaglioOrdine(ProspettoElektraWire model)
        {
            /*
            ...
            try
            {
                ...
                if (model.IdProspettoElektraWire != 0)
                {
                ...
                else
                {
                    ...
                    model.IdProspettoElektraWire = nuovoId;//03/04/2026 NUOVA
                    ...
                    //model.IdProspettoElektraWire = nuovoId;//03/04/2026 REMMATA
            ...
            //return RedirectToAction("DettaglioOrdine", new { id = model.IdProspettoElektraWire });//03/04/2026 resto nel dettaglio
            //return RedirectToAction("ListaOrdini", new { id = model.IdProspettoElektraWire });//03/04/2026 remmata
        }
        ...
        private void CaricaLookup(ProspettoElektraWire ordine)
        {
            // Agenti
            var agenti = _agenteRepository.GetAll();
        ...
        //// ============================================================
        //// NUOVA RIGA ORDINE ElektraWire (SOLO ADMIN)
        //// ============================================================
        public IActionResult Nuovo()
        {
            int idMandatario = HttpContext.Session.GetInt32("IdMandatario") ?? 0;

            var model = new ProspettoElektraWire();

            // --- CHIAVI E METADATI OBBLIGATORI ---
            model.IdMandatario = idMandatario;
            model.IdFileImportato = 0;
            model.IdImportElektraWire = 0;
            model.IdCliente = 0;
            model.IdMateriale = 0;
            ...
            model.Misure = string.Empty;

            // --- QUANTITÀ E PREZZI (DECIMAL 18,6) ---
            ...
            model.DeltaMargine = 0m;
            model.Perc_Delta_Provvigione = 0m; // Aggiunto 27/04/2026

            // --- BLOCCHI PROVVIGIONALI ELEKTRAWIRE 1.5 ---
            model.ValUnitProvvigioneFissa1_5 = 0m;
            model.Delta1ProvvigioneFissa1_5 = 0m;
            model.Delta2ProvvigioneFissa1_5 = 0m;
            model.TotProvvigioneFissa1_5 = 0m;
            model.ValQuotaFissa_1_5 = 0m;

            // --- BLOCCHI PROVVIGIONALI ELEKTRAWIRE 03 ---
            model.ValUnitProvvigioneFissa_03 = 0m;
            model.Delta1ProvvigioneFissa_03 = 0m;
            model.Delta2ProvvigioneFissa_03 = 0m;
            model.TotProvvigioneFissa_03 = 0m;
            model.ValQuotaFissa_03 = 0m;

            // --- TOTALI PROVVIGIONI ---
            model.TotaleFissoProvvigione = 0m;
            model.DeltaResiduoVariabile = 0m;
            model.TotaleVariabileProvvigione = 0m;
            model.TotaleSommaProvvigioni = 0m;
        ...
        public IActionResult DuplicaAjax(int id)//03/04/2026 modificata
        {
            ...
            return Json(new { success = true, newId = nuovo.IdProspettoElektraWire });
        }
        ...
        public IActionResult ExportExcelProspettoListaOrdini(int? idMandatario, int? idFileImportato, int? anno, int? mese, int? fatturata)
        {
            ...
            var nomeMandatario = "ElektraWire"; // oppure recuperalo dal DB
        ...
        public IActionResult ExportPdfProspettoListaOrdini(int? idMandatario, int? idFileImportato, int? anno, int? mese, int? fatturata)
        {
            ...
            var nomeMandatario = "ElektraWire"; // oppure lookup DB
            ...
            return File(bytes, "application/pdf", "ProspettoElektraWire.pdf");
        }
        ...
        public IActionResult ListaOrdiniJson(int? idMandatario, int? idFileImportato, int? anno, int? mese, int? fatturata, bool mostraEliminati = false)
        {
            ...
            // Il metodo GetFiltrati incapsula la chiamata alla Stored Procedure [dbo].[spProspettoElektraWire_GetAllFiltri].
            ...
            // VANTAGGIO ARCHITETTURALE: Restituendo l'intera collezione di oggetti 'ProspettoElektraWire',
            // non vi è accoppiamento forte (Hard Coding) tra le colonne del DB e il backend.
            // Qualsiasi colonna aggiuntiva (es. campi tecnici ElektraWire) sarà automaticamente disponibile 
